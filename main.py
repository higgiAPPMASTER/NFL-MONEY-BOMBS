...
async def get_skill_players(team_id: str) -> List[Dict]:
    """Get skill position players (QB/RB/WR/TE) for an ESPN team ID."""
    try:
        async with httpx.AsyncClient(timeout=15) as c:
...
async def get_career_vs_opp(player_id: str, opp: str, is_home: bool,
    ...
    """Career H/A game values vs specific opponent across all seasons."""
    values = []
    async def fetch(season):
        logs = await get_game_logs(player_id, season)
        for sp in logs:
            if sp.get("home") != is_home: continue
            if not _match(sp.get("opp", ""), opp): continue
            v = _get_stat(sp, stat_key)
...
async def run_pipeline(date_str: str) -> Dict:
    ...
    for game in games:
        ...
        for side, team_id, team_name, opp_name in [
            ("HOME", game["home_id"], home_team, away_team),
            ("AWAY", game["away_id"], away_team, home_team),
        ]:
            is_home  = (side == "HOME")
            players  = await get_skill_players(team_id)

            for player in players[:20]:   # cap per team to keep runtime reasonable
                pid  = player["id"]
                name = player["name"]
                pos  = player["pos"]
                ...
                for stat in STAT_DEFS:
                    ...
                    # Get career H/A values vs today's opponent
                    values = await get_career_vs_opp(pid, opp_name, is_home, sk)
                    if len(values) < MIN_GAMES:
                        continue

                    # Determine the line
                    odds_key  = _norm(name)
                    line      = (odds_lines.get(odds_key, {}).get(sk)
                                 if not is_past else None)
                    line_src  = "Odds API" if line else "Season Avg"
                    if not line:
                        line = await get_season_avg(pid, sk)
                    if not line or line < stat["min_line"]:
                        continue
...
.date-input{background:#161616;color:#f59e0b;border:1px solid #262626;border-radius:8px;padding:7px 12px;font-size:.85rem;font-weight:600;outline:none;cursor:pointer;font-family:'Source Sans Pro',sans-serif}
.btn{background:#f59e0b;color:#000;font-weight:700;padding:9px 20px;border:none;border-radius:8px;font-size:.85rem;cursor:pointer;font-family:'Source Sans Pro',sans-serif;transition:all .2s;white-space:nowrap}
