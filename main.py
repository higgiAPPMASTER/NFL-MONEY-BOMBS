...
async def run_pipeline(date_str: str) -> Dict:
    ...
    async def analyze_team(game, side, team_id, opp_name, is_home):
        ...
        await asyncio.gather(*[analyze_player(p) for p in players[:6]])

    # Run all games + both teams fully in parallel
    tasks = []
    for game in games:
        tasks.append(analyze_team(game, "HOME", game["home_id"], game["away_team"], True))
        tasks.append(analyze_team(game, "AWAY", game["away_id"], game["home_team"], False))
