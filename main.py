...
# ── ESPN Historical Schedule ───────────────────────────────────────────
async def get_espn_nfl_games(date_str: str) -> List[Dict]:
    """Get NFL games from ESPN for any date — works for past and future."""
...
async def run_pipeline(date_str: str) -> Dict:
    ...
    # Try Odds API first, fall back to ESPN schedule for past dates
    events = await get_nfl_events(date_str)
    if not events and is_past:
        # Odds API historical may not be available — use ESPN schedule instead
        events = await get_espn_nfl_games(date_str)
    if not events:
    ...
    # If no prop lines (ESPN fallback or off-season) — build picks from career stats only
    if not all_lines:
        if not is_past:
            return {"picks":[],"all":[],"error":"No prop lines posted yet — check back closer to game time"}
        # Historical demo mode: use default prop targets per position
        default_props = [
            {"market":"player_pass_yds","label":"Pass Yds","stat_key":"passingYards","line":250.5},
            {"market":"player_rush_yds","label":"Rush Yds","stat_key":"rushingYards","line":65.5},
            {"market":"player_reception_yds","label":"Rec Yds","stat_key":"receivingYards","line":55.5},
        ]
        for ev in events:
            for prop in default_props:
                l = dict(prop)
                l["name"]="";l["home_team"]=ev.get("home_team","");l["away_team"]=ev.get("away_team","");l["game"]=f"{ev.get('away_team','')} @ {ev.get('home_team','')}"
                all_lines.append(l)
        # Mark as demo
        return {"picks":[],"all":[],"error":f"No sportsbook lines available for {date_str}. Historical Odds API data requires a paid plan upgrade at the-odds-api.com","games":len(events),"matchups":[f"{ev.get('away_team','')} @ {ev.get('home_team','')}" for ev in events]}
    all_results=[]
...
.date-input{background:#0a0a0a;border:1px solid #2a2a2a;border-radius:8px;padding:11px 16px;color:#fff;font-size:14px;font-family:'Source Sans Pro',sans-serif;outline:none;transition:border .2s;flex:1}
...
<nav>
  ...
  <div style="font-size:12px;color:#6b7280">NFL Player Props</div>
</nav>
<main>
  <div class="hero">
...
function renderResults(data){
  ...
  if(data.error){
    ...
    if(data.matchups && data.matchups.length){
      matchupHtml = '<div style="margin-top:16px;display:flex;flex-wrap:wrap;gap:8px;justify-content:center">'
        + data.matchups.map(m=>'<div style="background:#161616;border:1px solid #262626;border-radius:8px;padding:8px 16px;font-size:13px;color:#9ca3af">🏈 '+m+'</div>').join('')
        + '</div>';
    }
    el.innerHTML='<div class="card no-games"><div class="icon">🏈</div><h3>'+data.error+'</h3>'+matchupHtml+'</div>';
