...
try:
    from nfl_props import run_nfl_props
    _NFL_PROPS_ENABLED = True
except ImportError:
    _NFL_PROPS_ENABLED = False
...
async def run_analysis(selected_date: str = None) -> Dict:
    ...
    # Build team schedule for props lookup
    team_sched = {}
    for game in games:
    ...
    if _NFL_PROPS_ENABLED:
        try:
            ...
            log.append(f"⚡ Props: {len(nfl_props_result.get('picks', []))} picks found")
        except Exception as e:
            log.append(f"⚠️ Props skipped: {e}")

    result = {'date': today_str, 'picks': top_picks, 'all_picks': picks,
              'games': games, 'log': log, 'total': len(picks),
              'nfl_props': nfl_props_result}
