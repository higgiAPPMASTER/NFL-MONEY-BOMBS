...
async def get_nfl_events(date_str: str) -> List[Dict]:
    """Fetch NFL events for a specific date (YYYY-MM-DD)."""
    if not ODDS_API_KEY: return []
    try:
        # Build date range for selected day (UTC)
...
async def get_prop_lines(event_id: str) -> List[Dict]:
    if not ODDS_API_KEY: return []
    try:
        async with httpx.AsyncClient(timeout=15) as c:
            r = await c.get(
...
async def run_pipeline(date_str: str) -> Dict:
    ...
    all_lines = []
    for ev in events:
        lines = await get_prop_lines(ev["id"])
        for l in lines:
