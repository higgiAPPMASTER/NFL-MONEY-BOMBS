...
# ── Odds API ──────────────────────────────────────────────────────────
async def get_nfl_events(date_str: str = None) -> List[Dict]:
    if not ODDS_API_KEY:
        return []
    try:
        params = {"apiKey": ODDS_API_KEY, "dateFormat": "iso"}
        if date_str:
            # Filter to events on the selected date (commenceTimeFrom/To)
            params["commenceTimeFrom"] = f"{date_str}T00:00:00Z"
            params["commenceTimeTo"]   = f"{date_str}T23:59:59Z"
        async with httpx.AsyncClient(timeout=15) as c:
            r = await c.get(f"{ODDS_BASE}/sports/americanfootball_nfl/events", params=params)
...
# ── Pipeline ──────────────────────────────────────────────────────────
async def run_nfl_props(date_str: str = None) -> Dict:
    if not ODDS_API_KEY:
...
async def api_run(request: Request):
    ...
    async def _run():
        try:
            result = await run_nfl_props(date_str)
...
<main>
  ...
  <div class="card">
    <div style="display:flex;gap:12px;align-items:center;margin-bottom:14px">
      <div style="flex:1">
        <label style="display:block;font-size:11px;font-weight:700;color:#6b7280;text-transform:uppercase;letter-spacing:.08em;margin-bottom:6px">Game Date</label>
        <input type="date" id="datePicker" style="background:#0a0a0a;border:1px solid #2a2a2a;border-radius:8px;padding:10px 14px;color:#fff;font-size:14px;width:100%;outline:none;font-family:inherit" />
      </div>
    </div>
...
document.addEventListener('DOMContentLoaded', function(){
  ...
  dp.value = today;
  dp.max = new Date(Date.now() + 7*86400000).toISOString().split('T')[0]; // +7 days
});
...
async function runPicks(){
  ...
  try {
    ...
    const r = await fetch('/api/run', {method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({date:dateStr})});
