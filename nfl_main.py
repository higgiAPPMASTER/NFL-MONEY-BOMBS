...
HIT_RATE_MIN = 0.75
MIN_GAMES    = 2
ESPN_SEASONS = [2025, 2024, 2023]   # NFL uses season start year
TOP_N        = 10
...
async def get_team_roster_espn(team_id: str) -> List[Dict]:
    ...
    try:
        async with httpx.AsyncClient(timeout=20) as c:
            ...
            data = r.json()
        players = []
        # Roster may be grouped by position group or flat list
        athletes = data.get('athletes', [])
        for item in athletes:
            if isinstance(item, dict):
                if 'items' in item:
                    for p in item['items']:
                        players.append({'id': p['id'], 'name': p.get('displayName','')})
                elif 'id' in item:
                    players.append({'id': item['id'], 'name': item.get('displayName','')})
        return players
